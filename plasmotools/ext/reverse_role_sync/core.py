...
class RRSCore(commands.Cog):
    ...
    async def audit_listener(self, entry: disnake.AuditLogEntry):
        ...
        # print("New audit entry captured:",
        #       entry.guild,  # digital drugs technologies | Plasmo RP
        #       entry.user,  # hffk#0
        #       entry.action,  #  AuditLogAction.member_role_update
        #       entry.before,  # <AuditLogDiff roles=[]>
        #       entry.after,  # <AuditLogDiff roles=[<Role id=1001980910842945606 name='Главный банкит'>]>
        #       entry.reason,  # None
        #       entry.changes,  # <AuditLogChanges before=<AuditLogDiff roles=[]> after=<AuditLogDiff roles=[<Role id=1001980910842945606 name='Главный банкит'>]>>
        #       entry.extra,  # None
        #       entry.target,  # vrtkk#0
        #       entry.category,  # AuditLogActionCategory.update
        #       entry.created_at,  # 2023-06-17 00:27:20.879000+00:00
        #       entry)
