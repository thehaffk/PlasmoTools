...
class BACSynchronization(commands.Cog):
    ...
    async def everyone_sync(self, inter: disnake.ApplicationCommandInteraction):
        ...
        embed_counter = disnake.Embed(
            title=("Синхронизация | " + str(inter.guild)), color=0xFFFF00
        )
    ...
    @commands.slash_command(
        name="sync", guild_ids=[settings.bac["id"]], auto_sync=False
    )
    @commands.has_permissions(manage_roles=True)
    async def sync_user(
        ...
        embed_counter = disnake.Embed(
            title=(f"Синхронизация {user} | " + str(inter.guild)), color=0xFFFF00
        )
        await inter.response.send_message(embed=embed_counter, ephemeral=True)
        await self.sync(user)

    @disnake.ext.commands.message_command(
        name="Sync", default_permission=True, guild_ids=[settings.bac["id"]]
    )
